...
def start(message):
    ...
    bot.reply_to(message, "Привет! Я ИИ-помощник, который умеет писать продающие описания товаров для Avito, Wildberries и Ozon, а ещё могу поддержать разговор на любую тему.\n\nНапиши мне название товара и детали, получишь готовый текст. Или просто задай любой вопрос.\n\nУ тебя " + str(user_free_left[uid]) + " бесплатных запросов. Дальше доступна Подписка, команда /podpiska")
...
def help_command(message):
    bot.reply_to(message, "Что умеет этот бот:\n\nЯ ИИ-помощник, который особенно хорошо пишет продающие описания товаров для Avito, Wildberries и Ozon. Разбираюсь в брендах, моделях кроссовок, технике и других товарах. А ещё можешь просто пообщаться со мной на любую тему.\n\nКак пользоваться:\n1. Напиши название товара и детали, или просто любой вопрос\n2. Получи ответ\n3. Можешь попросить: перепиши короче, добавь характеристики, сделай под Wildberries\n4. Команда /new начинает разговор заново\n\nКоманды:\n/start - начать сначала\n/new - новый разговор\n/podpiska - оформить Подписку\n/about - о нас\n/support - поддержка\n/myid - узнать свой Telegram ID")

@bot.message_handler(commands=['about'])
def about_command(message):
    bot.reply_to(message, "О нас:\n\nЭтот бот создан, чтобы продавцам на маркетплейсах не приходилось тратить часы на написание описаний товаров, а заодно может быть полезным ИИ-помощником на любые темы.")

@bot.message_handler(commands=['support'])
def support_command(message):
    bot.reply_to(message, "Если нашли ошибку или есть проблема с ботом, пишите сюда: @" + OWNER_USERNAME)
...
def buy_stars(call):
    ...
    bot.send_invoice(
        ...
        invoice_payload="podpiska_1_month",
        provider_token="",
        currency="XTR",
        prices=prices
    )
...
def generate(message):
    ...
    if not is_unlimited(uid) and is_new_topic:
        ...
        if user_free_left[uid] <= 0:
            bot.reply_to(message, "Бесплатные запросы закончились. Напиши /podpiska чтобы оформить Подписку с безлимитом.")
            return
