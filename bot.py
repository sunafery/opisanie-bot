...
def about_command(message):
    bot.reply_to(message, "О нас:\n\nМы сделали этого бота, потому что задолбались сами писать описания товаров вручную — на это уходят часы, которые продавец мог бы потратить на сам бизнес.\n\nТеперь это занимает 10 секунд: пишешь название и пару деталей, получаешь готовый продающий текст, который реально знает матчасть — бренды, модели, материалы. Не сухой шаблон, а текст, который хочется дочитать до конца.\n\nИспользуют продавцы на Avito, Wildberries и Ozon, которым важно не терять время и продавать быстрее.")
